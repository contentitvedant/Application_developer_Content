...
// It returns a new array containing only the items for which the condition item.name !== itemName is true, effectively
...
// Function to assign prototype
Inventory.prototype.assignPrototype = function(itemName, newPrototype) {
  const item = this.findItem(itemName);
  if (item) {
    item.__proto__ = newPrototype;
    console.log(`Prototype of item ${itemName} has been changed.`);
  } else {
...
7. Exit`);
    ...
    switch (choice) {
      ...
      case '5':
        ...
        break;
      case '6':
        const itemName = prompt('Enter the name of the item to assign a new prototype: ');
        const newPrototype = {};  // For simplicity, we're using an empty object as the new prototype. This can be customized.
        inventory.assignPrototype(itemName, newPrototype);
        break;
      case '7':
        console.log('Exiting...');
        process.exit();
